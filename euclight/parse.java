...
//    private double samples[] = new double[numSamples];
    ...
    void playBeep(final ALARM_TYPE type) {
    ...
    private void prepareTone(final int freq){

        for (int i = 0; i < numSamples; ++i)
        {
            final double originalWave = Math.sin(2 * Math.PI * freq * i / sampleRate);
            final double harmonic1 = 0.5 * Math.sin(2 * Math.PI * 2 * freq * i / sampleRate);
            final double harmonic2 = 0.25 * Math.sin(2 * Math.PI * 4 * freq * i / sampleRate);
            final double secondWave = Math.sin(2 * Math.PI * freq*1.34F * i / sampleRate);
            final double thirdWave = Math.sin(2 * Math.PI * freq*2.0F * i / sampleRate);
            final double fourthWave = Math.sin(2 * Math.PI * freq*2.68F * i / sampleRate);
...
/*        for (int i = 0; i < 20*numSamples/50; ++i)
    ...
    public void setBtName(final String btName) {
        mBtName = btName;
    }

    public void updateLight(final boolean enabledLight) {
	...
	public void updateLed(final boolean enabledLed) {
	...
	public void updatePedalsMode(final int pedalsMode) {
	...
	public void updateLightMode(final int lightMode) {
		if (mWheelType == WHEEL_TYPE.GOTWAY) {
			switch (lightMode) {
				case 0:
	...
	public void updateStrobe(final int strobeMode) {
		if (mWheelType == WHEEL_TYPE.KINGSONG) {
            final byte[] data = new byte[20];
            data[0] = (byte) 0xAA;
            data[1] = (byte) 0x55;
	...
	public void updateLedMode(final int ledMode) {
		if (mWheelType == WHEEL_TYPE.KINGSONG) {
            final byte[] data = new byte[20];
            data[0] = (byte) 0xAA;
            data[1] = (byte) 0x55;
	...
	public void updateAlarmMode(final int alarmMode) {
		if (mWheelType == WHEEL_TYPE.GOTWAY) {
			switch (alarmMode) {
				case 0:
	...
	public void updateHandleButton(final boolean enabledButton) {
	...
	public void updateMaxSpeed(final int wheelMaxSpeed) {
		...
		if (mWheelType == WHEEL_TYPE.GOTWAY) {
			...
			if (wheelMaxSpeed != 0) {
				final int wheelMaxSpeed2 = wheelMaxSpeed;
    ...
    public void updateKSAlarm1(final int wheelKSAlarm1) {
    ...
    public void updateKSAlarm2(final int wheelKSAlarm2) {
    ...
    public void updateKSAlarm3(final int wheelKSAlarm3) {
	...
	public void updateSpeakerVolume(final int speakerVolume) {
	...
	public void updatePedals(final int pedalAdjustment) {
    ...
    public String getRideTimeString() {
        final int currentTime = mRideTime + mLastRideTime;
        final long hours = TimeUnit.SECONDS.toHours(currentTime);
        final long minutes = TimeUnit.SECONDS.toMinutes(currentTime) -
                TimeUnit.HOURS.toMinutes(TimeUnit.SECONDS.toHours(currentTime));
        final long seconds = TimeUnit.SECONDS.toSeconds(currentTime) -
                TimeUnit.MINUTES.toSeconds(TimeUnit.SECONDS.toMinutes(currentTime));
        return String.format(Locale.US, "%02d:%02d:%02d", hours, minutes, seconds);
    }

	String getRidingTimeString() {
        final long hours = TimeUnit.SECONDS.toHours(mRidingTime);
        final long minutes = TimeUnit.SECONDS.toMinutes(mRidingTime) -
                TimeUnit.HOURS.toMinutes(TimeUnit.SECONDS.toHours(mRidingTime));
        final long seconds = TimeUnit.SECONDS.toSeconds(mRidingTime) -
	...
	public double getUserDistanceDouble() {
		if (mUserDistance == 0 && mTotalDistance != 0 )  {
			final Context mContext = mBluetoothLeService.getApplicationContext();
    ...
    public void setBetterPercents(final boolean betterPercents) {
    ...
    public void setBmsView(final boolean bmsView){
...
//    }
    ...
    void setAlarmsEnabled(final boolean enabled) {
        mAlarmsEnabled = enabled;
    }
	
	void setUseRatio(final boolean enabled) {
        mUseRatio = enabled;
		//reset();
    }

    void set18Lkm(final boolean enabled) {
        m18Lkm = enabled;
	...
	void setGotwayVoltage(final int voltage) {
        mGotwayVoltageScaler = voltage;
    }

    void setGotwayNegative(final int negative) {
        mGotwayNegative = negative;
    }

    void setPreferences(final int alarm1Speed, final int alarm1Battery,
                                   final int alarm2Speed, final int alarm2Battery,
                                   final int alarm3Speed, final int alarm3Battery,
                                   final int alarmCurrent,final int alarmTemperature,
                                    final boolean disablePhoneVibrate, final boolean disablePhoneBeep,
                                    final boolean alteredAlarms, final int rotationSpeed, final int rotationVoltage,
                                    final int powerFactor, final int alarmFactor1, final int alarmFactor2, final int alarmFactor3, final int warningSpeed
                                                                                                ) {
    ...
    private int byteArrayInt2(final byte low, final byte high) {
        return (low & 255) + ((high & 255) * 256);
    }

    private long byteArrayInt4(final byte value1, final byte value2, final byte value3, final byte value4) {
        return (((((long) ((value1 & 255) << 16))) | ((long) ((value2 & 255) << 24))) | ((long) (value3 & 255))) | ((long) ((value4 & 255) << 8));
    }

    private void setDistance(final long distance) {
    ...
    private void setCurrentTime(final int currentTime) {
        if (mRideTime > (currentTime + TIME_BUFFER))
            mLastRideTime += mRideTime;
        mRideTime = currentTime;
    }

    private void setTopSpeed(final int topSpeed) {
        if (topSpeed > mTopSpeed)
            mTopSpeed = topSpeed;
    }

    private void setVoltageSag(final int voltSag) {
        if ((voltSag < mVoltageSag) && (voltSag > 0))
            mVoltageSag = voltSag;
    }

    private void setBatteryPercent(final int battery) {
        mBattery = battery;
...
//        mAverageBattery += (battery - mAverageBattery) / mAverageBatteryCount;
    ...
    private void startSpeedAlarmCount() {
        ...
        final TimerTask stopSpeedAlarmExecuring = new TimerTask() {
            @Override
            public void run() {
        ...
        final Timer timerCurrent = new Timer();
    ...
    private void startTempAlarmCount() {
        ...
        final TimerTask stopTempAlarmExecuting = new TimerTask() {
            @Override
            public void run() {
    ...
    private void startCurrentAlarmCount() {
        ...
        final TimerTask stopCurrentAlarmExecuring = new TimerTask() {
            @Override
            public void run() {
    ...
    private void playWarningSpeed(final Context mContext) {
        final MediaPlayer mp1 = MediaPlayer.create(mContext, R.raw.sound_warning_speed);
        mp1.start();
        mp1.setOnCompletionListener(mp11 -> mp11.release());
    }

    private void checkAlarmStatus(final Context mContext) {
        // SPEED ALARM
        if (!mSpeedAlarmExecuting) {
            if (mAlteredAlarms) {
                final double resultFactor = ((float)mSpeed/100.0)/((mRotationSpeed/mRotationVoltage) * ((float)mVoltage/100.0) * mPowerFactor);
...
*/
    ...
    private void raiseAlarm(final ALARM_TYPE alarmType, final Context mContext) {
        final Vibrator v = (Vibrator) mContext.getSystemService(Context.VIBRATOR_SERVICE);
        long[] pattern = {0};
        final Intent intent = new Intent(Constants.ACTION_ALARM_TRIGGERED);
...
//                mCurrentAlarmExecuted = true;
    ...
    void decodeResponse(final byte[] data, final Context mContext) {
        mDataForLog = true;
...
//        FileUtil.writeLine("bluetoothOutput.txt", stringBuilder.toString());
        ...
        final boolean new_data = getAdapter().decode(data);

        if (!new_data)
			return;

		final Intent intent = new Intent(Constants.ACTION_WHEEL_DATA_AVAILABLE);
		if (mDataForLog) {
    ...
    public boolean decodeKingSong(final byte[] data) {
        if (rideStartTime == 0) {
            rideStartTime = Calendar.getInstance().getTimeInMillis();
			mRidingTime = 0;
		}
        if (data.length >= 20) {
            final int a1 = data[0] & 255;
            final int a2 = data[1] & 255;
            if (a1 != 170 || a2 != 85) {
                return false;
            }
            if ((data[16] & 255) == 169) { // Live data
                ...
                final long distance = byteArrayInt4(data[2], data[3], data[4], data[5]);
                setDistance(distance);
                //int currentTime = byteArrayInt2(data[6], data[7]);
	            final int currentTime = (int) (Calendar.getInstance().getTimeInMillis() - rideStartTime) / 1000;
    ...
    public boolean decodeGotway(final byte[] data) {
        ...
        if (data.length >= 20) {
            ...
            final int a1 = data[0] & 255;
            final int a2 = data[1] & 255;
            final int a3 = data[2] & 255;
            final int a4 = data[3] & 255;
            final int a5 = data[4] & 255;
            final int a6 = data[5] & 255;
            final int a19 = data[18] & 255;
            if ((a1 == 0xDC) && (a2 == 0x5A) && (a3 == 0x5C) && (a4 == 0x20)) {  // Sherman
                ...
                final long distance = ((data[10] & 0xFF) << 24 | (data[11] & 0xFF) << 16 | (data[8] & 0xFF) << 8 | (data[9] & 0xFF));
                setDistance(distance);
    ...
    public boolean decodeNinebotZ(final byte[] data) {
        NinebotZAdapter.getInstance().setBmsReadingMode(mBmsView);
        final ArrayList<NinebotZAdapter.Status> statuses = NinebotZAdapter.getInstance().charUpdated(data);
        if (statuses.size() < 1) return false;
        if (rideStartTime == 0) {
            rideStartTime = Calendar.getInstance().getTimeInMillis();
            mRidingTime = 0;
        }
        for (final NinebotZAdapter.Status status: statuses) {
    ...
    public boolean decodeNinebot(final byte[] data) {
        final ArrayList<NinebotAdapter.Status> statuses = NinebotAdapter.getInstance().charUpdated(data);
        if (statuses.size() < 1) return false;
        if (rideStartTime == 0) {
            rideStartTime = Calendar.getInstance().getTimeInMillis();
            mRidingTime = 0;
        }
        for (final NinebotAdapter.Status status: statuses) {
            Timber.i(status.toString());
    ...
    public boolean decodeInmotion(final byte[] data) {
        final ArrayList<InMotionAdapter.Status> statuses = InMotionAdapter.getInstance().charUpdated(data);
		if (statuses.size() < 1) return false;
        if (rideStartTime == 0) {
            rideStartTime = Calendar.getInstance().getTimeInMillis();
			mRidingTime = 0;
		}		
        for (final InMotionAdapter.Status status: statuses) {
            Timber.i(status.toString());
    ...
    boolean detectWheel(final BluetoothLeService bluetoothService, final String deviceAddress) {
        //audioTrack.write(buffer, 20000, buffer.length);

        mBluetoothLeService = bluetoothService;
        final Context mContext = bluetoothService.getApplicationContext();
        final String advData = SettingsUtil.getAdvDataForWheel(mContext,deviceAddress);
         //String wheel_Type = "";
        protoVer = "";
        ...
        final Class<R.array> res = R.array.class;
        final String wheel_types[] = mContext.getResources().getStringArray(R.array.wheel_types);
        for (String wheel_Type : wheel_types) {
            ...
            final String services[] = mContext.getResources().getStringArray(services_res_id);

            if (services.length != mBluetoothLeService.getSupportedGattServices().size())
                continue;

            for (final String service_uuid : services) {
                final UUID s_uuid = UUID.fromString(service_uuid.replace("_", "-"));
                final BluetoothGattService service = mBluetoothLeService.getGattService(s_uuid);
                if (service != null) {
                    ...
                    final String characteristics[] = mContext.getResources().getStringArray(characteristic_res_id);
                    for (final String characteristic_uuid : characteristics) {
                        final UUID c_uuid = UUID.fromString(characteristic_uuid.replace("_", "-"));
                        final BluetoothGattCharacteristic characteristic = service.getCharacteristic(c_uuid);
            ...
            if (detected_wheel) {
                ...
                if (mContext.getResources().getString(R.string.kingsong).equals(wheel_Type)) {
                    ...
                    final BluetoothGattService targetService = mBluetoothLeService.getGattService(UUID.fromString(Constants.KINGSONG_SERVICE_UUID));
                    final BluetoothGattCharacteristic notifyCharacteristic = targetService.getCharacteristic(UUID.fromString(Constants.KINGSONG_READ_CHARACTER_UUID));
                    mBluetoothLeService.setCharacteristicNotification(notifyCharacteristic, true);
                    final BluetoothGattDescriptor descriptor = notifyCharacteristic.getDescriptor(UUID.fromString(Constants.KINGSONG_DESCRIPTER_UUID));
                    ...
                    final BluetoothGattService targetService = mBluetoothLeService.getGattService(UUID.fromString(Constants.GOTWAY_SERVICE_UUID));
                    final BluetoothGattCharacteristic notifyCharacteristic = targetService.getCharacteristic(UUID.fromString(Constants.GOTWAY_READ_CHARACTER_UUID));
                    mBluetoothLeService.setCharacteristicNotification(notifyCharacteristic, true);
                    ...
                    final BluetoothGattService targetService = mBluetoothLeService.getGattService(UUID.fromString(Constants.INMOTION_SERVICE_UUID));
                    final BluetoothGattCharacteristic notifyCharacteristic = targetService.getCharacteristic(UUID.fromString(Constants.INMOTION_READ_CHARACTER_UUID));
                    mBluetoothLeService.setCharacteristicNotification(notifyCharacteristic, true);
                    final BluetoothGattDescriptor descriptor = notifyCharacteristic.getDescriptor(UUID.fromString(Constants.INMOTION_DESCRIPTER_UUID));
                    descriptor.setValue(BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE);
                    mBluetoothLeService.writeBluetoothGattDescriptor(descriptor);
                    if (SettingsUtil.hasPasswordForWheel(mContext, mBluetoothLeService.getBluetoothDeviceAddress())) {
                        final String inmotionPassword = SettingsUtil.getPasswordForWheel(mBluetoothLeService.getApplicationContext(), mBluetoothLeService.getBluetoothDeviceAddress());
                    ...
                    final BluetoothGattService targetService = mBluetoothLeService.getGattService(UUID.fromString(Constants.NINEBOT_Z_SERVICE_UUID));
                    Timber.i("service UUID");
                    final BluetoothGattCharacteristic notifyCharacteristic = targetService.getCharacteristic(UUID.fromString(Constants.NINEBOT_Z_READ_CHARACTER_UUID));
                    Timber.i("read UUID");
                    ...
                    final BluetoothGattDescriptor descriptor = notifyCharacteristic.getDescriptor(UUID.fromString(Constants.NINEBOT_Z_DESCRIPTER_UUID));
                    Timber.i("descr UUID");
                    if (descriptor == null) {
                    ...
                    mWheelType = WHEEL_TYPE.NINEBOT;
                    final BluetoothGattService targetService = mBluetoothLeService.getGattService(UUID.fromString(Constants.NINEBOT_SERVICE_UUID));
                    Timber.i("service UUID");
                    final BluetoothGattCharacteristic notifyCharacteristic = targetService.getCharacteristic(UUID.fromString(Constants.NINEBOT_READ_CHARACTER_UUID));
                    Timber.i("read UUID");
                    if (notifyCharacteristic == null) {
                        Timber.i("it seems that RX UUID doesn't exist");
                    }
                    mBluetoothLeService.setCharacteristicNotification(notifyCharacteristic, true);
                    Timber.i("notify UUID");
                    final BluetoothGattDescriptor descriptor = notifyCharacteristic.getDescriptor(UUID.fromString(Constants.NINEBOT_DESCRIPTER_UUID));
                    Timber.i("descr UUID");
                    if (descriptor == null) {
                        Timber.i("it seems that descr UUID doesn't exist");
                    }
